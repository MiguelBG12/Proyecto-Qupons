...
CREATE TABLE `usuario` (
  ...
  `update_usuarioid` INT,
  `update_datetime` DATETIME,
  `delete_usuarioid` INT,
  `delete_datetime` DATETIME,
  PRIMARY KEY (`usuario_id`)
  )
/*INSERT INTO usuario(nombres_completos, dni, genero, fecha_nacimiento, direccion, departamento, correo, contraseña, telefono) VALUES ('Mario', 12345678, 'masculino', '1958-01-30', 'Av La gardenias 1345', 'lima', 'mario@gmail.com', 'dsjfno234', 9756123 )usuario*/
...
CREATE TABLE `cliente_tienda` (
  ...
  `insert_datetime` DATETIME,
  `update_clienteid` INT,
  `update_datetime` DATETIME,
  `delete_clienteid` INT,
  `delete_datetime` DATETIME,
...
CREATE TABLE `cupones` (
  `cupones_id` INT NOT NULL AUTO_INCREMENT, #expect a random number 
...
CREATE TABLE `cupon_comprado` (
  `cupon_comprado_id` INT NOT NULL AUTO_INCREMENT, #expect a random number
  `fecha_compra` DATE NOT NULL,
  `codigo_venta` INT NOT NULL, #expect a random number
  `cantidad_comprada` INT NOT NULL,
  `estado_cupon` ENUM('canjeado', 'no canjeado', 'vencido') NOT NULL,
  `usuario_id` INT NOT NULL,
  `cupones_id` INT NOT NULL,
  PRIMARY KEY (`cupon_comprado_id`),
  INDEX `fk_CUPON COMPRADO_USUARIO_idx` (`usuario_id` ASC) VISIBLE,
  INDEX `fk_CUPON COMPRADO_CUPONES1_idx` (`cupones_id` ASC) VISIBLE,
  CONSTRAINT `fk_CUPON COMPRADO_USUARIO`
    FOREIGN KEY (`usuario_id`)
    REFERENCES `db_webcuponera`.`usuario` (`usuario_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_CUPON COMPRADO_CUPONES1`
